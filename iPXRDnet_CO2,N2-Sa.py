...
import csv,json,copy,torch,re,os
...
SET_load=torch.load('data/Sa_sp.pt')
train_SET,dev_SET,test_SET=SET_load[0],SET_load[1],SET_load[2]
set_train = []
set_val = []
set_test = []

for InputInfo in adinfo_list_new:
    if InputInfo[0] in train_SET:
        set_train.append(InputInfo)
    elif InputInfo[0] in dev_SET:
        set_val.append(InputInfo)
    elif InputInfo[0] in test_SET:
        set_test.append(InputInfo)
...
torch.save(best_model.state_dict(),'Sa-CO2,N2.pt')
