...
def build_model(tgt,embed_dim=1024, dropout=0.05):
    model = Encoder3D(XrdCNN(tgt=tgt), RegressionHead(300*3+128*1,2048, len(tgt), last_dropout=dropout))
    return model
...
class Encoder3D(nn.Module):
    ...
    def forward_once(self, xrd_input,anion_input,ligand_input,M_input,gas_input):
        ...
        all_encoder = torch.cat([ligand_embed_ed,anion_embed_ed ], dim=-1)
