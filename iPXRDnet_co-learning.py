...
class XrdCNN(nn.Module):
    def __init__(self, tgt):
        ...
        # Dense layers
        self.fc1 = nn.Linear(26624, 512)  # Adjusted according to the output size from global average pooling
...
set_train_origin = [adinfo_list_new[i] for i in train_idx]
set_val = [adinfo_list_new[i] for i in val_idx]
set_test = [adinfo_list_new[i] for i in test_idx]


range_num=np.geomspace(5e-05, 1, num=15)
list_num_mul=[]
for num_mul in range_num[0:11]:
    trainsize=int(len(set_train_origin)*num_mul)
    list_num_mul.append(trainsize)
sequence =  ['95','795','20000','40000','60000','228367']
...
for trainsize in sequence:
    print(trainsize)
    set_train=set_train_origin[:trainsize]
    data_y_train=[]
