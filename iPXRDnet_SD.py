...
class XrdCNN(nn.Module):
    def __init__(self, tgt):
        ...
        # Dense layers
        self.fc1 = nn.Linear(26624+128, 512)  
...
adinfo_list_new=torch.load('data/GAS_SD-data.pt')
gas_dict=torch.load('data/gas_dict.pt')
...
for epoch in range(epochs):
    ...
    for item in train_loader:
        ...
        gas_input=item[2].to(device)
        y_hat = model(data_xrd,gas_input)
...
for i in range(0,len(tgt),1):
    ...
    print(f'Test {target_dict[tgt[i]]} R2: {r2:.3f}, MAE: {mae:.3f}')
print("{}\t TEST mse:{:.3f} r2 {:.5f}".format(i+1,mse,r2.item()))
torch.save(model.state_dict(),'GAS_SD-7:3-120E-0.15DP.pt')
