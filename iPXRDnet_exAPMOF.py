...
class RegressionHead(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(input_dim, inner_dim)  # Adjusted according to the output size from global average pooling
...
def build_model(tgt,embed_dim=1024, dropout=0.05):
    model = EncoderAll(XrdCNN(tgt=tgt), RegressionHead(embed_dim+300*3+128*1,2048, len(tgt), last_dropout=dropout))
    return model
...
all_exAPMOF=torch.load('data/all_exAPMOF-1bar.pt')
...
savename='exAPMOF-1bar-ALM+PXRD.pt'
