...
from script_for_models.utils_dist import set_seed,GradCAM,visualize_heatmap
def filter_by_gas(set_train, TGT_gas):
    filtered_list = []
    
    for entry in set_train:
        # Check if the gas in the entry matches the target gas
        if entry[1] == TGT_gas:
...
for InputInfo in set_train:
    key=InputInfo[0]
    ad_data=[0,0,0,0]
    ad_data[GAS_index[InputInfo[1]]]=float(InputInfo[-1])
...
for epoch in range(epochs):
    ...
    for item in train_loader:
        ...
        for indexnum in range(len(item[0])):
            ...
            ad_data=[0,0,0,0]
            ad_data[GAS_index[item[1][indexnum]]]=float(item[-1][indexnum])
    ...
    if((epoch+1)%10 == 0):
        ...
        print("time: {:.5f}".format(time() - t2))
        t2 = time()
        try:
            for i in range(0,len(tgt),1):
                r2 = round(r2_score(test_label_best[:, i].detach().cpu().numpy(), test_pred_best[:, i].detach().cpu().numpy()), 3)
                mae = mean_absolute_error(test_label_best[:, i].detach().cpu().numpy(), test_pred_best[:, i].detach().cpu().numpy())
    ...
    if mse < bestmse:
        ...
        try:
            ...
            train_pred = torch.cat(train_pred).to(device)
            print(epoch,optim.param_groups[0]['lr'] * 1e5)
            for i in range(0,len(tgt),1):
                train_pred[:, i] = train_pred[:, i] * scales[i][1] + scales[i][0]
                train_label[:, i] = train_label[:, i] * scales[i][1] + scales[i][0]
        ...
        test_label_best = torch.cat(test_label).to(device)
        test_pred_best = torch.cat(test_pred).to(device)
        for i in range(0,len(tgt),1):
            test_pred_best[:, i] = test_pred_best[:, i] * scales[i][1] + scales[i][0]
            test_label_best[:, i] = test_label_best[:, i] * scales[i][1] + scales[i][0]
            r2 = round(r2_score(test_label_best[:, i].detach().cpu().numpy(), test_pred_best[:, i].detach().cpu().numpy()), 3)
            mae = mean_absolute_error(test_label_best[:, i].detach().cpu().numpy(), test_pred_best[:, i].detach().cpu().numpy())
            print(f'Dev {target_dict[tgt[i]]} R2: {r2:.3f}, MAE: {mae:.3f}')
        print("{}\t Dev mse:{:.3f} r2 {:.5f}".format(i+1,mse,r2.item()))
...
for item in set_test:
    if item[0]=='hMOF-1001364':
        set_PIC.append(item)
print(len(set_PIC),set_PIC[0])
PIC_loader = DataLoader(set_PIC, batch_size=50)
model.eval()
test_label=[]
test_pred=[]
mse=0
with torch.no_grad():
    for item in PIC_loader: 
        Xrd_list,ADSTR_list,Gas_list,Pressure_list=[],[],[],[]
        for indexnum in range(len(item[0])):
            XRD_INFO=XRD_DICT[item[0][indexnum]]
            Xrd_list.append(XRD_INFO)
...
INDEX_HAET=0
example_heatmap = heatmapCH4[INDEX_HAET].cpu().numpy()
visualize_heatmap(example_heatmap,'CH4.png')
INDEX_HAET=0
example_heatmap = heatmapN2[INDEX_HAET].cpu().numpy()
visualize_heatmap(example_heatmap,'N2.png')
INDEX_HAET=0
example_heatmap = heatmapCO2[INDEX_HAET].cpu().numpy()
visualize_heatmap(example_heatmap,'CO2.png')
INDEX_HAET=0
example_heatmap = heatmapH2[INDEX_HAET].cpu().numpy()
visualize_heatmap(example_heatmap,'H2.png')
