...
if __name__ == "__main__":
    ...
    model=XrdCNN().cuda()
    [adinfo_list, pxrd_data, ads_gasEMB]=torch.load('data-up/iMOF_PXRD,AD.pt')
    predictions, true_values = material_wise_loo(
        adinfo_list, pxrd_data, ads_gasEMB, model
    )
    plot_performance(true_values, predictions)
    
    #For data of uptake less than 6 mol/kg
    model=XrdCNN().cuda()
    [adinfo_list, pxrd_data, ads_gasEMB]=torch.load('data-up/iMOF_PXRD,AD.pt')
    adinfo_list2=[]
    for item in adinfo_list:
        if item[2]<7:
            adinfo_list2.append(item)
    print(len(adinfo_list2))
    predictions, true_values = material_wise_loo(
        adinfo_list2, pxrd_data, ads_gasEMB, model
    )
    plot_performance(true_values, predictions)