...
def get_cmpn_encoder(args: Namespace) -> CMPN:
    """
    创建并返回一个CMPN编码器。
    ...
    model = MoleculeModel(classification=args.dataset_type == 'classification', 
                          ...
                          pretrain=False)
    model.create_encoder(args, 'CMPNN')
    ...
    def forward(self, atom_hiddens: torch.Tensor, fg_states: torch.Tensor, atom_num, fg_indexs):
        ...
        if self.args.cuda:
            for i in range(len(fg_indexs)):
                fg_states.scatter_(0, fg_indexs[i:i+1], self.cls.cuda())
        else:
            for i in range(len(fg_indexs)):
...
def encoder_add_functional_prompt(encoder, args):
    encoder.encoder.W_i_atom = prompt_generator_output(args)(encoder.encoder.W_i_atom)
    return encoder
