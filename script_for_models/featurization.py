...
relative_path = os.path.join(current_dir, 'initial', 'ele2emb.pkl')
#print(relative_path)
ele2emb = pickle.load(open(relative_path,'rb'))
def ele_features(ele):
    fele = ele2emb[ele]
    return fele.tolist()

current_dir = os.path.dirname(os.path.abspath(__file__))
relative_path = os.path.join(current_dir, 'initial', 'rel2emb.pkl')
rel2emb = pickle.load(open(relative_path,'rb'))
def relation_features(e1,e2):
    frel = rel2emb[(e1,e2)]
    return frel.tolist()

current_dir = os.path.dirname(os.path.abspath(__file__))
relative_path = os.path.join(current_dir, 'initial', 'funcgroup.txt')
...
current_dir = os.path.dirname(os.path.abspath(__file__))
relative_path = os.path.join(current_dir, 'initial', 'fg2emb.pkl')
fg2emb = pickle.load(open(relative_path, 'rb'))
def match_fg(mol):
    fg_emb = [[1] * 133]
    pad_fg = [[0] * 133]
    for sm in smart:
